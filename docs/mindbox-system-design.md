    ...
    └── {project_id}/
        ...
        └── tasks/
            └── {task_id}/
                ...
                │   └── logs/              # 脚本执行日志（重定向的 stdout/stderr）
                ├── logs/
                │   ├── stdout.log         # 标准输出
...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/health` | 系统状态（当前 task、GPU 信息、Kernel 类型） |
...
为避免训练脚本的大量 stdout/stderr 直接进入 Agent 上下文窗口，Kernel 在启动 Claude Code 前会在 task 目录下生成 `AGENT.md`（包含脚本执行与日志诊断规则），并创建 `CLAUDE.md -> AGENT.md` 软链接。Claude Code 会自动读取 `CLAUDE.md` 并遵循其中约束，因此脚本输出**必须**重定向到 `workspace/logs/` 下的日志文件，再通过读取日志尾部进行诊断与结果提取。
