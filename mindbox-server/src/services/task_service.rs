...
use chrono::Utc;
use mindbox_common::{
    ArtifactItem, CreateTaskRequest, LogLevel, Metric, MindboxError, Result, Task, TaskEvent,
...
impl TaskService {
    ...
    pub async fn create_and_start_task(&self, req: CreateTaskRequest) -> Result<Task> {
        ...
        tokio::spawn(async move {
            if let Err(e) = run_kernel(state, task_for_run, task_dir_for_run).await {
    ...
    pub async fn get_task(&self, task_id: &str) -> Result<Task> {
        let task_file = self.task_dir(task_id).join("task.yaml");
        if !task_file.exists() {
    ...
    pub async fn retry_task(&self, task_id: &str) -> Result<Task> {
        ...
        self.create_and_start_task(CreateTaskRequest {
    ...
    pub async fn read_logs(&self, task_id: &str) -> Result<String> {
        self.get_task(task_id).await?;

        let logs_path = self.task_dir(task_id).join("logs/kernel.log");
        if !logs_path.exists() {
    ...
    pub async fn list_artifacts(&self, task_id: &str) -> Result<Vec<ArtifactItem>> {
        self.get_task(task_id).await?;

        let artifacts_dir = self.task_dir(task_id).join("artifacts");
    ...
    pub async fn artifact_path(&self, task_id: &str, path: &str) -> Result<PathBuf> {
        self.get_task(task_id).await?;

        let base = self.task_dir(task_id).join("artifacts");
    ...
    pub fn tasks_dir(&self) -> PathBuf {
        self.state.config.tasks_dir()
    }

    async fn save_task(&self, task: &Task) -> Result<()> {
        let task_file = self.task_dir(&task.id).join("task.yaml");
    ...
    async fn generate_task_id(&self) -> Result<String> {
...
impl BroadcastCallback {
    fn new(task_id: String, tx: tokio::sync::broadcast::Sender<BroadcastEvent>) -> Self {
        Self { task_id, tx }
    }
...
async fn run_kernel(state: AppState, task: Task, task_dir: PathBuf) -> Result<()> {
...
mod tests {
    ...
    async fn task_id_format_uses_sequence() {
        ...
        tokio::fs::create_dir_all(service.tasks_dir())
            .await
            .expect("mkdir");

        let id = service.generate_task_id().await.expect("id");
