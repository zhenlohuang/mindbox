use axum::{
    Json, Router,
    extract::{Path, State},
    routing::{get, post},
};
use mindbox_common::{
    CancelTaskResponse, CreateTaskRequest, GetTaskResponse, ListTasksResponse, RetryTaskResponse,
};

use crate::{error::ApiResult, services::task_service::TaskService, state::AppState};

pub fn router() -> Router<AppState> {
    Router::new()
        .route("/api/v1/tasks", post(create_task).get(list_tasks))
        .route("/api/v1/tasks/{task_id}", get(get_task))
        .route("/api/v1/tasks/{task_id}/cancel", post(cancel_task))
        .route("/api/v1/tasks/{task_id}/retry", post(retry_task))
}

async fn create_task(
    State(state): State<AppState>,
    Json(req): Json<CreateTaskRequest>,
) -> ApiResult<Json<GetTaskResponse>> {
    let service = TaskService::new(state);
    let task = service.create_and_start_task(req).await?;
    Ok(Json(GetTaskResponse { task }))
}

async fn list_tasks(State(state): State<AppState>) -> ApiResult<Json<ListTasksResponse>> {
    let service = TaskService::new(state);
    let tasks = service.list_tasks().await?;
    Ok(Json(ListTasksResponse { tasks }))
}

async fn get_task(
    State(state): State<AppState>,
    Path(task_id): Path<String>,
) -> ApiResult<Json<GetTaskResponse>> {
    let service = TaskService::new(state);
    let task = service.get_task(&task_id).await?;
    Ok(Json(GetTaskResponse { task }))
}

async fn cancel_task(
    State(state): State<AppState>,
    Path(task_id): Path<String>,
) -> ApiResult<Json<CancelTaskResponse>> {
    let service = TaskService::new(state);
    let task = service.cancel_task(&task_id).await?;
    Ok(Json(CancelTaskResponse {
        task_id: task.id,
        status: task.status,
    }))
}

async fn retry_task(
    State(state): State<AppState>,
    Path(task_id): Path<String>,
) -> ApiResult<Json<RetryTaskResponse>> {
    let service = TaskService::new(state);
    let task = service.retry_task(&task_id).await?;
    Ok(Json(RetryTaskResponse { task }))
}
